...
import DebugPanel, { triggerLogoClick } from './components/DebugPanel';
...
function App() {
  ...
  useEffect(() => {
    const unsub = onError((info) => {
      setErrorInfo((prev) => prev || info);
    });

    const handleApiError = (e) => {
      setErrorInfo((prev) => prev || {
        type: 'api_error',
    ...
    const handleReactError = (e) => {
      setErrorInfo((prev) => prev || e.detail);
    };
  ...
  const appStateSnapshot = useMemo(() => ({
    ...
    loading,
    rewriting,
  }), [text, file, result, rewriteResult, rewriteLevel, quota, loading, rewriting]);
  ...
  const handleFileUpload = async (e) => {
    ...
    if (quota <= 0) {
      ...
      clientLogger.warn('Quota exceeded on file upload');
      return;
    }
    ...
    try {
      const response = await api.detectFile(selectedFile);
      setResult(response);
  ...
  const handleDetectText = async () => {
    ...
    if (quota <= 0) {
      alert("今日额度已用完，请明天再试或升级账户。");
      clientLogger.warn('Quota exceeded on detect');
      return;
    }
    ...
    setRewriteResult(null);
    try {
      const response = await api.detectText(text);
      setResult(response);
      decreaseQuota();
  ...
  const handleRewrite = async () => {
    ...
    clientLogger.addUserPath(`rewrite:${rewriteLevel}`);

    if (quota <= 0) {
      alert("今日额度已用完，请明天再试或升级账户。");
      clientLogger.warn('Quota exceeded on rewrite');
      return;
    }

    setRewriting(true);
    try {
      const response = await api.rewrite(text, rewriteLevel);
